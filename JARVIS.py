...
import wave
import numpy as np 
from phonenumbers import Phonenumber_location_tracker
...
class MainThread(QThread):
    ...
    def run_jarvis(self):
        self.wish()
        self.talk('HOW MAY I HELP YOU, SIR')
        while True:
            ...
            if ('play a song' in self.command) or ('youtube' in self.command) or ("download a song" in self.command) or ("download song" in self.command) : 
                ...
                self.talk("Thanks for using me Sir, have a good day")
                sys.exit()
                ...
                self.talk("Please wait Sir, featching the latest news")
                self.news()
    ...
    def qrCodeGenerator(self):
        ...
        self.talk(f"Sir the qr code has been generated")
    ...
    def comum(self,command):
        ...
        if ('hi'in command) or('hai'in command) or ('hey'in command) or ('hello' in command) :
            self.talk("Hello Sir how may I help for u")
        else :
    ...
    def Fun(self,command):
        ...
        if 'your name' in command:
            ...
            self.talk("your name is Durvesh a k a Dhruv")
        elif 'university name' in command:
            self.talk("you are studying in mumbai unversity fot bacelor in computer science") 
            ...
            self.talk('People live their lives bound by what they accept as correct and true. Thats how they define reality. But what does it mean to be correct or true? Merely vague concepts.Ones reality might be others illusion. We all live inside our own fantasies. ')
    ...
    def social(self,command):
        ...
        if 'facebook' in command:
            ...
            webbrowser.open('https://www.instagram.com/sudo_self/')
    ...
    def schedule(self):
        ...
        Week = {"monday" : "Sir, you have a Cyber Forensics Lecture at 7:15, so you must be awake by 5 a.m. - Train on a 6:45 a.m.- train status 2 minutes ahead of time.",
        "tuesday" : " Sir, You have a cyber forensics lecture from 7:15 to 8:50, then a second W.S.N lecture until 10:30, so you must get up at 5 a.m. to catch a train at 6:45 a.m.",
        "wednesday" : "Sir today you have a full day of classes from 7:15 to 8:50 you have D.I.P class, from 9:00 to 9:50 you have Data Science class, from 10:00 to 11:20 you have W.S.N class",
        "thrusday" : "Sir, You have a data science practical today... sir, please don't skip this class because your attendance is less than 60%.",
        "friday" : "Sir, Today You have two Data Image Processing Lecture afterward W.S.N section",
        "saturday" : "Sir, you have a full day of classes today. From 7:00 to 8:05, you have DS, followed by an Ethical Hacking lecture.",
        "sunday":"Sir today is holiday but we can't say anything when they will bomb with any assisgnments and i would suggest doing some exercise because you getting fat, Sir"}
        if day in Week.keys():
            self.talk(Week[day])
    ...
    def open_source(self,command):
        ...
        if 'github' in command:
            ...
            webbrowser.open('https://github.com/D-Sensei')
    ...
    def CloseApp(self,command):
        print(command)
        if ('calculator'in command) :
            self.talk("okay Sir, closeing caliculator")
            os.system("taskkill /f /im calc.exe")
        elif ('paint'in command) :
            self.talk("okay Sir, closeing mspaint")
            os.system("taskkill /f /im mspaint.exe")
        elif ('notepad'in command) :
            self.talk("okay Sir, closeing notepad")
            os.system("taskkill /f /im notepad.exe")
        elif ('discord'in command) :
            self.talk("okay Sir, closeing discord")
            os.system("taskkill /f /im Discord.exe")
        elif ('editor'in command) :
            self.talk("okay Sir, closeing vs code")
            os.system("taskkill /f /im Code.exe")
        elif ('spotify'in command) :
            self.talk("okay Sir, closeing spotify")
            os.system("taskkill /f /im Spotify.exe")
        elif ('lt spice'in command) :
            self.talk("okay Sir, closeing lt spice")
            os.system("taskkill /f /im XVIIx64.exe")
        elif ('steam'in command) :
            self.talk("okay Sir, closeing steam")
            os.system("taskkill /f /im steam.exe")
        elif ('media player'in command) :
            self.talk("okay Sir, closeing media player")
    ...
    def news(self):
        MAIN_URL_= "https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=7b2c0d459f4f4ad9a47b7535014f00ba"
...
class Main(QMainWindow):
    ...
    def startTask(self):
        ...
        self.ui.movie.start()
        self.ui.movie = QtGui.QMovie(rf"{self.cpath}\UI\dhruv.gif")
